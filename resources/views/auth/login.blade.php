...
@section('content')
    <form class="card card-md" action="{{ route('login') }}" method="post" autocomplete="off">
        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                <label class="form-label">{{ __('Email / Phone') }}</label>
                <input type="text" name="login" value="{{ old('login') }}" class="form-control @error('login') is-invalid @enderror" placeholder="{{ __('Enter email Or Phone Number') }}" required autofocus tabindex="1">
                @error('login')
